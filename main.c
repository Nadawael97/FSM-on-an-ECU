...
void Output_init()
{
	...
	GPIOPinTypeGPIOOutput(GPIO_PORTA_BASE, 0x01 );
...
int main(void)
{
	...
	while(1)
	{
        ...
        else
        {
			...
			if (temp == (LEFT_BUTTON))
			{
				GPIOPinWrite(GPIO_PORTA_BASE, 0x01, 0x01);
				LED_ON(RED_GPIO_PIN, RED_GPIO_PIN);
			}
			else if (temp == (RIGHT_BUTTON))
			{
				GPIOPinWrite(GPIO_PORTA_BASE, 0x01, 0x00);
				LED_ON(RED_GPIO_PIN, 0x00);
			}
		#endif
